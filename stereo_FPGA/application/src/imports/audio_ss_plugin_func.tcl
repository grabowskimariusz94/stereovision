...
# BELOW ARE ALL SD SPECIFIC FUNCTIONS
	...
	proc get_leaf_blocks {var_connect var_topology {upvar_num 1} {shadow_copy 1}} {
		...
		if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
			...
			set ss_connect($ss_topology,leaf) [list axi_smartconnect $clk_wiz aud_pat_gen hdmi_acr_ctrl]
		} else {
	...
	proc get_ss_connectivity {var_connect var_topology {upvar_num 1}} {
		...
		if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
			...
			set ss_connect($ss_topology,source2target,bdry_in,S00_AXI)           [list axi_smartconnect   S00_AXI]
			set ss_connect($ss_topology,source2target,bdry_in,ACLK)              [list axi_smartconnect   aclk \
																					aud_pat_gen        axi_aclk \
																					hdmi_acr_ctrl      axi_aclk \
																					$clk_wiz            s_axi_aclk \
																					$clk_wiz            clk_in1]
			set ss_connect($ss_topology,source2target,bdry_in,ARESETN)           [list axi_smartconnect   aresetn \
		...
		if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
			lappend source_objs axi_smartconnect
			set ss_connect($ss_topology,source2target,axi_smartconnect,M00_AXI)   [list aud_pat_gen     axi]
			set ss_connect($ss_topology,source2target,axi_smartconnect,M01_AXI)   [list hdmi_acr_ctrl   axi]
			set ss_connect($ss_topology,source2target,axi_smartconnect,M02_AXI)   [list $clk_wiz        s_axi_lite]
		} else {
	...
	proc config_obj_property {tobe_config_obj tobe_config_obj_path} {
		...
		switch -regexp -matchvar varlist -- $tobe_config_obj {
                ...
                axi_smartconnect    {
                                      ::ss_common::go_config $tobe_config_obj_path config_axi_smartconnect
                                    }
				clk_wiz             {
				                      ::ss_common::go_config $tobe_config_obj_path config_clk_wiz
				                    }
				clk_wizard          {
				                      ::ss_common::go_config $tobe_config_obj_path config_clk_wizard
				                    }
		}
