...
#        set ss_connect($ss_topology,bdry,pinlist)  $proc_bdry_pinlist          
        ...
        if {$var_exdes_topology == 3} {
			set ss_connect($ss_topology,bdry,pinlist) [list ext_reset_in hdmi_rx_irq hdmi_tx_irq vphy_irq vfbwr_irq vfbrd_irq vfbwr_arstn vfbrd_arstn\
															IIC clk_out2 dcm_locked peripheral_aresetn s_axi_aclk]
		} elseif {$var_exdes_topology == 4} {
			set ss_connect($ss_topology,bdry,pinlist) [list ext_reset_in hdmi_rx_irq hdmi_tx_irq vphy_irq i2s_rx_irq i2s_tx_irq\
															IIC clk_out2 dcm_locked peripheral_aresetn s_axi_aclk]		
		} else {
    ...
    proc get_leaf_blocks {var_connect var_topology {upvar_num 1} {shadow_copy 1}} {
		...
		if {$var_exdes_topology == 3} {
			...
			set sys_block_list [list axi_interconnect_1 axi_interconnect_0 xlconcat0 zynq_us fmch_axi_iic xlslice0 xlslice1\
									 rst_processor_1_100M rst_processor_1_300M]		
		} else {
			# ss_connect(<topology>,leaf) [<all the needed leaf object per topology>]   -- bug, cannot use common word
			set sys_block_list [list axi_interconnect_0 xlconcat0 zynq_us fmch_axi_iic \
									 rst_processor_1_100M rst_processor_1_300M]
		}
    ...
    proc get_ss_connectivity {var_connect var_topology {upvar_num 1}} {
		...
		if {$var_exdes_topology == 3} {
             set ss_connect($ss_topology,source2target,bdry_in,vfbwr_irq)      [list xlconcat0              In3]
             set ss_connect($ss_topology,source2target,bdry_in,vfbrd_irq)      [list xlconcat0              In4]
             set ss_connect($ss_topology,source2target,fmch_axi_iic,iic2intc_irpt)      [list xlconcat0              In5]
		}
		if {$var_exdes_topology == 4} {		
             set ss_connect($ss_topology,source2target,bdry_in,i2s_rx_irq)      [list xlconcat0              In3]
             set ss_connect($ss_topology,source2target,bdry_in,i2s_tx_irq)      [list xlconcat0              In4]
		}
        ...
        set ss_connect($ss_topology,source2target,rst_processor_1_100M,interconnect_aresetn)    [list axi_interconnect_0   ARESETN]
        ...
        for {set i 0} {$i < $var_axi_intercon_num_mi} {incr i} {
            ...
            set ss_connect($ss_topology,source2target,rst_processor_1_100M,peripheral_aresetn)    [concat [set ss_connect($ss_topology,source2target,rst_processor_1_100M,peripheral_aresetn)] \
                                                                                                             axi_interconnect_0    M0${i}_ARESETN]
            } elseif {$i > 9 && $i != 5 && $i != 8} {
            set ss_connect($ss_topology,source2target,rst_processor_1_100M,peripheral_aresetn)    [concat [set ss_connect($ss_topology,source2target,rst_processor_1_100M,peripheral_aresetn)] \
                                                                                                             axi_interconnect_0    M${i}_ARESETN]
            }
        }       
        ...
        set ss_connect($ss_topology,source2target,zynq_us,M_AXI_HPM0_LPD)   [list axi_interconnect_0      S00_AXI]    
        ...
        for {set i 0} {$i < $var_axi_intercon_num_mi} {incr i} {
            ...
            set ss_connect($ss_topology,source2target,zynq_us,pl_clk0)      [concat [set ss_connect($ss_topology,source2target,zynq_us,pl_clk0)] \
                                                                                        axi_interconnect_0     M0${i}_ACLK]
            } elseif {$i > 9 && $i != 5 && $i != 8} {
            set ss_connect($ss_topology,source2target,zynq_us,pl_clk0)      [concat [set ss_connect($ss_topology,source2target,zynq_us,pl_clk0)] \
                                                                                        axi_interconnect_0     M${i}_ACLK]
            }
        }       
        ...
        if {$var_exdes_topology == 3} {
			...
			set ss_connect($ss_topology,source2target,zynq_us,emio_gpio_o)          [list xlslice0  Din \
			                                                                              xlslice1  Din]																					 
        } else {
			set ss_connect($ss_topology,source2target,zynq_us,pl_clk1)          [list rst_processor_1_300M  slowest_sync_clk \
																					  axi_interconnect_0      M05_ACLK \
		...
		# For Video Frame Buffer Read and Video Framer Buffer Write
        if {$var_exdes_topology == 3} {
			set ss_connect($ss_topology,source2target,rst_processor_1_300M,interconnect_aresetn)  [list axi_interconnect_0      M05_ARESETN \
        ...
        lappend source_objs axi_interconnect_0
        set ss_connect($ss_topology,source2target,axi_interconnect_0,M04_AXI)    [list fmch_axi_iic     S_AXI]
		
        for {set i 0} {$i < $var_axi_intercon_num_mi} {incr i} {
            if {$i < 10 && $i != 3 && $i != 4 && $i != 7} {
            set ss_connect($ss_topology,source2target,axi_interconnect_0,M0${i}_AXI)    [list bdry_out     M0${i}_AXI]
            } elseif {$i > 9 && $i != 3 && $i != 4 && $i != 7} {
            set ss_connect($ss_topology,source2target,axi_interconnect_0,M${i}_AXI)     [list bdry_out     M${i}_AXI]
            }       
        }
        ...
        if {$var_exdes_topology == 3} {
			...
			set ss_connect($ss_topology,source2target,axi_interconnect_1,M00_AXI)    [list zynq_us     S_AXI_HP0_FPD]  	
        ...
        set ss_connect($ss_topology,sourceIsInterfacePin) [list M_AXI_HPM_LPD IIC M_AXI S_AXI]    
    ...
    proc config_obj_property {tobe_config_obj tobe_config_obj_path} {
        ...
        switch -regexp -matchvar varlist -- $tobe_config_obj {
                ...
                ^zynq_us            {
									  ...
									  if {$var_exdes_topology == 3} {
									       set config_zynq_us(CONFIG.PSU__FPGA_PL1_ENABLE)  1
                                           set config_zynq_us(CONFIG.PSU__USE__M_AXI_GP1)  0
                                           set config_zynq_us(CONFIG.PSU__USE__S_AXI_GP2)  1
                                           set config_zynq_us(CONFIG.PSU__CRL_APB__PL1_REF_CTRL__SRCSEL)  IOPLL
                                           set config_zynq_us(CONFIG.PSU__GPIO_EMIO__PERIPHERAL__ENABLE)  1
									  }
                                      
                                      ::ss_common::go_config $tobe_config_obj_path config_zynq_us
                                    }                                                       
                ^axi_interconnect_0   {
                                        set config_axi_interconnect_0(CONFIG.NUM_MI) $var_axi_intercon_num_mi
                                        ::ss_common::go_config $tobe_config_obj_path config_axi_interconnect_0
                                    }
                ^axi_interconnect_1 {
										if {$var_exdes_topology == 3} {
