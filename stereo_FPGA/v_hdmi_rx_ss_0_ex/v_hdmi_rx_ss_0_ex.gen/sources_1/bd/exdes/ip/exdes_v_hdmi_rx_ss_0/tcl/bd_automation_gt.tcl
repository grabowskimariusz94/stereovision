proc bd_automation {parentCell} {
  ...
  set gt_ip_inst [bd::gt_utils::find_connected_core [get_bd_intf_pins ${parentCell}/LINK_DATA0_IN]]
  set GT_QUAD [bd::gt_utils::find_connected_core [get_bd_intf_pins ${gt_ip_inst}/gt_rx0]]
  ...
  set RX_PLL_SELECTION [get_property CONFIG.C_RX_PLL_SELECTION $gt_ip_inst ]

  if { $RX_PLL_SELECTION == 7 } {
...
#  connect_bd_net [get_bd_pins ${gt_ip_inst}/gt_ch2_ilo_resetdone] [get_bd_pins $GT_QUAD/ch2_iloresetdone]
 ...
 # set lcplllock_pin_val [bd::gt_utils::find_connected_pin [get_bd_pins $GT_QUAD/hsclk0_lcplllock]]
 # if {$lcplllock_pin_val eq ""} {
 #   connect_bd_net [get_bd_pins ${gt_ip_inst}/gt_lcpll0_lock] [get_bd_pins $GT_QUAD/hsclk0_lcplllock]
 # } else {
 #   connect_bd_net [get_bd_pins ${gt_ip_inst}/gt_lcpll1_lock] [get_bd_pins $GT_QUAD/hsclk1_lcplllock]
 # } 
 # set lcpllrst_pin_val [bd::gt_utils::find_connected_pin [get_bd_pins $GT_QUAD/hsclk0_lcpllreset]]
 # if {$lcpllrst_pin_val eq ""} {
 #   connect_bd_net [get_bd_pins ${gt_ip_inst}/gt_lcpll0_reset] [get_bd_pins $GT_QUAD/hsclk0_lcpllreset]
 # } else {
 #   connect_bd_net [get_bd_pins ${gt_ip_inst}/gt_lcpll1_reset] [get_bd_pins $GT_QUAD/hsclk1_lcpllreset]
 # } 
...
##  } 
 ...
 # } 
  ...
  set bufg_gt_inst [bd::gt_utils::find_connected_core [get_bd_pins ${gt_ip_inst}/gt_rxusrclk]]
  connect_bd_net [get_bd_pins ${gt_ip_inst}/rx_axi4s_aclk] [get_bd_pins ${parentCell}/link_clk] [get_bd_pins $bufg_gt_inst/usrclk]
