...
# BELOW ARE ALL SD SPECIFIC FUNCTIONS
    ...
    proc get_bdry_pinlist {var_connect var_topology {upvar_num 1}} {
        ...
        #default
        set sys_bdry_pinlist  [list fmch_iic \
                                    TX_HPD_IN TX_DDC_OUT \
                                    TX_EN_OUT \
                                    RX_DET_IN RX_DDC_OUT \
                                    RX_HPD_OUT]

        if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
            lappend sys_bdry_pinlist HDMI_DATA
            lappend sys_bdry_pinlist TX_REFCLK_IN
            lappend sys_bdry_pinlist HDMI_RX_CLK_IN
            lappend sys_bdry_pinlist RX_REFCLK_OUT
            lappend sys_bdry_pinlist CH_DDR
        } else {
            lappend sys_bdry_pinlist HDMI_TX_CLK_P_OUT
            lappend sys_bdry_pinlist HDMI_TX_CLK_N_OUT
            lappend sys_bdry_pinlist HDMI_TX_DAT_P_OUT
            lappend sys_bdry_pinlist HDMI_TX_DAT_N_OUT
            lappend sys_bdry_pinlist HDMI_RX_DAT_P_IN
            lappend sys_bdry_pinlist HDMI_RX_DAT_N_IN
            lappend sys_bdry_pinlist TX_REFCLK_P_IN
            lappend sys_bdry_pinlist TX_REFCLK_N_IN
            lappend sys_bdry_pinlist HDMI_RX_CLK_P_IN
            lappend sys_bdry_pinlist HDMI_RX_CLK_N_IN
            lappend sys_bdry_pinlist RX_REFCLK_P_OUT
            lappend sys_bdry_pinlist RX_REFCLK_N_OUT
            lappend sys_bdry_pinlist reset
        }
        if {[regexp xilinx.com:zcu104 $var_board_name match] == 1 || [regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
            lappend sys_bdry_pinlist IDT_8T49N241_LOL_IN
        } else {
        ...
        if {[string equal -nocase $var_PROCESSOR_TYPE MicroBlaze] == 1} {
            ...
            if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
                lappend sys_bdry_pinlist sys_diff_clock
            }
        }
...
# baoshan: 12Aug - removed this as it configure the ports trigger CWs
    ...
    proc get_leaf_blocks {var_connect var_topology {upvar_num 1} {shadow_copy 1}} {
        ...
        if {[string equal -nocase $var_PROCESSOR_TYPE MicroBlaze] == 1} {
            ...
            set hdmiphy vid_phy_controller
        } elseif {[string equal -nocase $var_PROCESSOR_TYPE ARM] == 1} {
            set processor_name zynq_ss_0
            set hdmiphy vid_phy_controller
        } elseif {[string equal -nocase $var_PROCESSOR_TYPE ARM64] == 1} {
			if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
				set processor_name cips_ss_0
				set hdmiphy hdmiphy_ss_0
			} else {
		...
		if {$var_exdes_topology == 3} {
			...
			set sys_block_list [list $hdmiphy vcc_const v_frmbuf_wr v_frmbuf_rd\
									 tx_video_axis_reg_slice v_hdmi_tx_ss \
									 v_hdmi_rx_ss rx_video_axis_reg_slice]
		} else {
			# ss_connect(<topology>,leaf) [<all the needed leaf object per topology>]   -- bug, cannot use common word
			set sys_block_list [list $hdmiphy vcc_const \
        ...
        if {[string equal -nocase $var_PROCESSOR_TYPE MicroBlaze] == 1} {
            ...
            if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
                lappend sys_block_list  gt_refclk_buf_ss_0
                lappend sys_block_list  gt_refclk_buf_ss_1
                if {[string match -nocase $var_c_nidru true]} {
                    lappend sys_block_list  gt_refclk_buf_ss_2
                }
            }
        }
...
#       set ss_connect($ss_topology,leaf) [list audio_ss_0]
    ...
    proc get_ss_connectivity {var_connect var_topology {upvar_num 1}} {
        ...
        variable var_fid_enable
        set var_fid_enable 0
        ...
        if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
		    set_property CONFIG.FREQ_HZ 200000000 [get_bd_intf_ports /sys_diff_clock]
            set ss_connect($ss_topology,source2target,bdry_in,sys_diff_clock)   [list $processor_name     CLK_IN1_D]
        }
        ...
        if {[regexp xilinx.com:zcu104 $var_board_name match] == 1 || [regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
            set ss_connect($ss_topology,source2target,bdry_in,IDT_8T49N241_LOL_IN)      [list $hdmiphy  tx_refclk_rdy]
        } else {
            set ss_connect($ss_topology,source2target,bdry_in,SI5324_LOL_IN)            [list $hdmiphy  tx_refclk_rdy]
        }

        if {[string equal -nocase $var_PROCESSOR_TYPE MicroBlaze] == 1} {
            set ss_connect($ss_topology,source2target,bdry_in,TX_REFCLK_P_IN)   [list $hdmiphy  mgtrefclk1_pad_p_in]
            set ss_connect($ss_topology,source2target,bdry_in,TX_REFCLK_N_IN)   [list $hdmiphy  mgtrefclk1_pad_n_in]
            set ss_connect($ss_topology,source2target,bdry_in,HDMI_RX_CLK_P_IN) [list $hdmiphy  mgtrefclk0_pad_p_in]
            set ss_connect($ss_topology,source2target,bdry_in,HDMI_RX_CLK_N_IN) [list $hdmiphy  mgtrefclk0_pad_n_in]
            ...
            if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
                set ss_connect($ss_topology,source2target,gt_refclk_buf_ss_0,IBUFDSGT_OUT)         [list $hdmiphy  tx_ref_clk_in]
                set ss_connect($ss_topology,source2target,gt_refclk_buf_ss_0,IBUFDSGT_ODIV2_OUT)   [list $hdmiphy  tx_ref_clk_odiv2_in]
                set ss_connect($ss_topology,source2target,gt_refclk_buf_ss_1,IBUFDSGT_OUT)         [list $hdmiphy  rx_ref_clk_in]
                set ss_connect($ss_topology,source2target,gt_refclk_buf_ss_1,IBUFDSGT_ODIV2_OUT)   [list $hdmiphy  rx_ref_clk_odiv2_in]
            } else {
                set ss_connect($ss_topology,source2target,bdry_in,reset)            [list $processor_name     ext_reset_in]
                set ss_connect($ss_topology,source2target,bdry_in,TX_REFCLK_P_IN)   [list $hdmiphy  mgtrefclk0_pad_p_in]
                set ss_connect($ss_topology,source2target,bdry_in,TX_REFCLK_N_IN)   [list $hdmiphy  mgtrefclk0_pad_n_in]
                set ss_connect($ss_topology,source2target,bdry_in,HDMI_RX_CLK_P_IN) [list $hdmiphy  mgtrefclk1_pad_p_in]
                set ss_connect($ss_topology,source2target,bdry_in,HDMI_RX_CLK_N_IN) [list $hdmiphy  mgtrefclk1_pad_n_in]
            }
            if {[string match -nocase $var_c_nidru true]} {
                if {[string equal -nocase $var_GT_TYPE GTH] == 1} {
                    set ss_connect($ss_topology,source2target,bdry_in,DRU_CLK_IN)       [list gt_refclk_buf   CLK_IN_D]
                } elseif {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
                    set ss_connect($ss_topology,source2target,gt_refclk_buf_ss_2,IBUFDSGT_OUT)             [list $hdmiphy        dru_ref_clk_in]
                    set ss_connect($ss_topology,source2target,gt_refclk_buf_ss_2,IBUFDSGT_ODIV2_OUT)       [list $hdmiphy        dru_ref_clk_odiv2_in]
                }
            }
        }

        if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
            lappend source_objs gt_refclk_buf_ss_0
            set ss_connect($ss_topology,source2target,bdry_in,TX_REFCLK_IN)   [list gt_refclk_buf_ss_0  IBUFDSGT_IN]
            lappend source_objs gt_refclk_buf_ss_1
            set ss_connect($ss_topology,source2target,bdry_in,HDMI_RX_CLK_IN) [list gt_refclk_buf_ss_1  IBUFDSGT_IN]
            if {[string match -nocase $var_c_nidru true]} {
                lappend source_objs gt_refclk_buf_ss_2
                set ss_connect($ss_topology,source2target,bdry_in,DRU_CLK_IN)       [list gt_refclk_buf_ss_2        IBUFDSGT_IN]
            }
        } else {
            set ss_connect($ss_topology,source2target,bdry_in,HDMI_RX_DAT_P_IN) [list $hdmiphy  phy_rxp_in]
            set ss_connect($ss_topology,source2target,bdry_in,HDMI_RX_DAT_N_IN) [list $hdmiphy  phy_rxn_in]
        }

        lappend source_objs rx_video_axis_reg_slice
		if {$var_exdes_topology == 3} {
			set ss_connect($ss_topology,source2target,rx_video_axis_reg_slice,M_AXIS)        [list v_frmbuf_wr  s_axis_video]
		} else {
        ...
        set ss_connect($ss_topology,source2target,$processor_name,M00_AXI)        [list $hdmiphy   vid_phy_axi4lite]
		...
		if {$var_exdes_topology == 3} {
			set ss_connect($ss_topology,source2target,$processor_name,M05_AXI)        [list v_frmbuf_wr          s_axi_CTRL]
			set ss_connect($ss_topology,source2target,$processor_name,M08_AXI)        [list v_frmbuf_rd          s_axi_CTRL]
		} else {
        ...
        if {$var_exdes_topology == 3} {
			set s_axi_aresetn_list                                                      [list v_hdmi_tx_ss         s_axi_cpu_aresetn \
																							  v_hdmi_rx_ss         s_axi_cpu_aresetn \
																							  $hdmiphy   vid_phy_sb_aresetn \
																							  $hdmiphy   vid_phy_axi4lite_aresetn]
		} else {
        ...
        if {$var_exdes_topology == 3} {
			set s_axi_aclk_list                                                         [list v_hdmi_tx_ss         s_axi_cpu_aclk \
																							  v_hdmi_rx_ss         s_axi_cpu_aclk \
																							  $hdmiphy   vid_phy_sb_aclk \
																							  $hdmiphy   vid_phy_axi4lite_aclk]
		} else {
        ...
        if {[string equal -nocase $var_GT_TYPE GTH] == 1} {
            lappend s_axi_aclk_list                                                       $hdmiphy   drpclk
        } elseif {[string equal -nocase $var_GT_TYPE GTY] == 1} {
            lappend s_axi_aclk_list                                                       $hdmiphy   drpclk
        }

        if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
            set ss_connect($ss_topology,source2target,$processor_name,CH_DDR)             [list bdry_out  CH_DDR]
        }

		if {$var_exdes_topology == 3} {
			lappend source_objs v_frmbuf_wr
			set ss_connect($ss_topology,source2target,v_frmbuf_wr,m_axi_mm_video)          [list $processor_name  S00_AXI]

			lappend source_objs v_frmbuf_rd
			set ss_connect($ss_topology,source2target,v_frmbuf_rd,m_axi_mm_video)          [list $processor_name  S01_AXI]

			set ss_connect($ss_topology,source2target,v_frmbuf_rd,m_axis_video)          [list tx_video_axis_reg_slice  S_AXIS]

			lappend s_axi_aclk_list                                                        v_hdmi_tx_ss  s_axis_audio_aclk
			lappend s_axi_aclk_list                                                        v_hdmi_rx_ss  s_axis_audio_aclk
			...
			set ss_connect($ss_topology,source2target,v_tpg_ss_0,m_axis_video)          [list tx_video_axis_reg_slice  S_AXIS]
                        if {$var_fid_enable == 1} {
                            set ss_connect($ss_topology,source2target,v_tpg_ss_0,fid_out)               [list v_hdmi_tx_ss  fid]
                        }
        ...
        set ss_connect($ss_topology,source2target,v_hdmi_tx_ss,LINK_DATA0_OUT)              [list $hdmiphy    vid_phy_tx_axi4s_ch0]
        set ss_connect($ss_topology,source2target,v_hdmi_tx_ss,LINK_DATA1_OUT)              [list $hdmiphy    vid_phy_tx_axi4s_ch1]
        set ss_connect($ss_topology,source2target,v_hdmi_tx_ss,LINK_DATA2_OUT)              [list $hdmiphy    vid_phy_tx_axi4s_ch2]
        ...
        if {[string equal -nocase $var_PROCESSOR_TYPE MicroBlaze] == 1} {
            if {[string equal -nocase $var_GT_TYPE GTX] == 1} {
                if {[string match -nocase $var_c_nidru true]} {
                    ...
                    set ss_connect($ss_topology,source2target,gt_refclk_buf,IBUF_OUT) [list $hdmiphy  gtnorthrefclk0_in]
                }
            } elseif {[string equal -nocase $var_GT_TYPE GTH] == 1} {
                if {[string match -nocase $var_c_nidru true]} {
                    lappend source_objs dru_ibufds_gt_odiv2
                    set ss_connect($ss_topology,source2target,dru_ibufds_gt_odiv2,BUFG_GT_O) [list $hdmiphy  gtnorthrefclk0_odiv2_in]
                    lappend source_objs gt_refclk_buf
                    if {$var_C_RX_PLL_SELECTION == 6} {
                        set ss_connect($ss_topology,source2target,gt_refclk_buf,IBUF_OUT) [list $hdmiphy  gtnorthrefclk0_in \
                                                                                                $hdmiphy  gtnorthrefclk00_in \
                                                                                                $hdmiphy  gtnorthrefclk01_in]
                    } else {
                        set ss_connect($ss_topology,source2target,gt_refclk_buf,IBUF_OUT) [list $hdmiphy  gtnorthrefclk0_in]
                    }
            ...
            if {[string equal -nocase $var_GT_TYPE GTX] == 1} {
                ...
                set ss_connect($ss_topology,source2target,gt_refclk_buf0,IBUF_OUT) [list    $hdmiphy  gtsouthrefclk0_in]
                if {[string match -nocase $var_c_nidru true]} {
                    lappend source_objs gt_refclk_buf1
                    set ss_connect($ss_topology,source2target,gt_refclk_buf1,IBUF_OUT) [list    $hdmiphy  gtsouthrefclk1_in]
                }
            }
        } elseif {[string equal -nocase $var_PROCESSOR_TYPE ARM64] == 1} {
            if {[string equal -nocase $var_GT_TYPE GTH] == 1} {
                if {[string match -nocase $var_c_nidru true]} {
                    ...
                    if {[string match -nocase $var_DRU_REFCLK gtsouthrefclk0]} {
                        set ss_connect($ss_topology,source2target,dru_ibufds_gt_odiv2,BUFG_GT_O) [list $hdmiphy  gtsouthrefclk0_odiv2_in]
                        lappend source_objs gt_refclk_buf
                        if {$var_C_RX_PLL_SELECTION == 6} {
                            set ss_connect($ss_topology,source2target,gt_refclk_buf,IBUF_OUT) [list $hdmiphy  gtsouthrefclk0_in \
                                                                                                    $hdmiphy  gtsouthrefclk00_in \
                                                                                                    $hdmiphy  gtsouthrefclk01_in]
                        } else {
                            set ss_connect($ss_topology,source2target,gt_refclk_buf,IBUF_OUT) [list $hdmiphy  gtsouthrefclk0_in]
                        }
                    } elseif {[string match -nocase $var_DRU_REFCLK gtnorthrefclk1]} {
                        set ss_connect($ss_topology,source2target,dru_ibufds_gt_odiv2,BUFG_GT_O) [list $hdmiphy  gtnorthrefclk1_odiv2_in]
                        lappend source_objs gt_refclk_buf
                        if {$var_C_RX_PLL_SELECTION == 6} {
                            set ss_connect($ss_topology,source2target,gt_refclk_buf,IBUF_OUT) [list $hdmiphy  gtnorthrefclk1_in \
                                                                                                    $hdmiphy  gtnorthrefclk10_in \
                                                                                                    $hdmiphy  gtnorthrefclk11_in]
                        } else {
                            set ss_connect($ss_topology,source2target,gt_refclk_buf,IBUF_OUT) [list $hdmiphy  gtnorthrefclk1_in]
                        }
                    } else {
                        set ss_connect($ss_topology,source2target,dru_ibufds_gt_odiv2,BUFG_GT_O) [list $hdmiphy  gtsouthrefclk1_odiv2_in]
                        lappend source_objs gt_refclk_buf
                        if {$var_C_RX_PLL_SELECTION == 6} {
                            set ss_connect($ss_topology,source2target,gt_refclk_buf,IBUF_OUT) [list $hdmiphy  gtsouthrefclk1_in \
                                                                                                    $hdmiphy  gtsouthrefclk10_in \
                                                                                                    $hdmiphy  gtsouthrefclk11_in]
                        } else {
                            set ss_connect($ss_topology,source2target,gt_refclk_buf,IBUF_OUT) [list $hdmiphy  gtsouthrefclk1_in]
                        }
                    }
        ...
        lappend source_objs $hdmiphy
        set ss_connect($ss_topology,source2target,$hdmiphy,vid_phy_rx_axi4s_ch0)          [list v_hdmi_rx_ss  LINK_DATA0_IN]
        set ss_connect($ss_topology,source2target,$hdmiphy,vid_phy_rx_axi4s_ch1)          [list v_hdmi_rx_ss  LINK_DATA1_IN]
        set ss_connect($ss_topology,source2target,$hdmiphy,vid_phy_rx_axi4s_ch2)          [list v_hdmi_rx_ss  LINK_DATA2_IN]
        set ss_connect($ss_topology,source2target,$hdmiphy,vid_phy_status_sb_tx)          [list v_hdmi_tx_ss  SB_STATUS_IN]
        set ss_connect($ss_topology,source2target,$hdmiphy,vid_phy_status_sb_rx)          [list v_hdmi_rx_ss  SB_STATUS_IN]
        if {$var_exdes_topology != 3} {
            set ss_connect($ss_topology,source2target,$hdmiphy,tx_tmds_clk)               [list audio_ss_0    hdmi_clk]
        }
        set ss_connect($ss_topology,source2target,$hdmiphy,tx_video_clk)                  [list v_hdmi_tx_ss  video_clk]
        if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
            set ss_connect($ss_topology,source2target,$hdmiphy,rx_tmds_clk)                   [list bdry_out      RX_REFCLK_OUT]
        } else {
            set ss_connect($ss_topology,source2target,$hdmiphy,tx_tmds_clk_p)                 [list bdry_out      HDMI_TX_CLK_P_OUT]
            set ss_connect($ss_topology,source2target,$hdmiphy,tx_tmds_clk_n)                 [list bdry_out      HDMI_TX_CLK_N_OUT]
            set ss_connect($ss_topology,source2target,$hdmiphy,rx_tmds_clk_p)                 [list bdry_out      RX_REFCLK_P_OUT]
            set ss_connect($ss_topology,source2target,$hdmiphy,rx_tmds_clk_n)                 [list bdry_out      RX_REFCLK_N_OUT]
        }
        set ss_connect($ss_topology,source2target,$hdmiphy,rx_video_clk)                  [list v_hdmi_rx_ss  video_clk]
        if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
            set ss_connect($ss_topology,source2target,$hdmiphy,phy_data)                      [list bdry_out      HDMI_DATA]
          #  set ss_connect($ss_topology,source2target,$hdmiphy,phy_tx_out)                    [list bdry_out      HDMI_TX_DAT_OUT]
        } else {
            set ss_connect($ss_topology,source2target,$hdmiphy,phy_txn_out)                   [list bdry_out      HDMI_TX_DAT_N_OUT]
            set ss_connect($ss_topology,source2target,$hdmiphy,phy_txp_out)                   [list bdry_out      HDMI_TX_DAT_P_OUT]
        }
        if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
            set ss_connect($ss_topology,source2target,$hdmiphy,rxoutclk)                      [list v_hdmi_rx_ss        link_clk]
            set ss_connect($ss_topology,source2target,$hdmiphy,txoutclk)                      [list v_hdmi_tx_ss        link_clk]
        } else {
            set ss_connect($ss_topology,source2target,$hdmiphy,rxoutclk)                      [list $hdmiphy            vid_phy_rx_axi4s_aclk \
                                                                                                    v_hdmi_rx_ss        link_clk]
            set ss_connect($ss_topology,source2target,$hdmiphy,txoutclk)                      [list $hdmiphy            vid_phy_tx_axi4s_aclk \
                                                                                                    v_hdmi_tx_ss        link_clk]
        }
        set ss_connect($ss_topology,source2target,$hdmiphy,irq)                           [list $processor_name     vphy_irq]
		...
		if {$var_exdes_topology == 3} {
			set ss_connect($ss_topology,source2target,v_frmbuf_wr,interrupt)               [list $processor_name          vfbwr_irq]
			set ss_connect($ss_topology,source2target,v_frmbuf_rd,interrupt)               [list $processor_name          vfbrd_irq]
			set ss_connect($ss_topology,source2target,$processor_name,vfbwr_arstn)         [list v_frmbuf_wr              ap_rst_n]
			set ss_connect($ss_topology,source2target,$processor_name,vfbrd_arstn)         [list v_frmbuf_rd              ap_rst_n]
			set ss_connect($ss_topology,source2target,v_hdmi_rx_ss,fid)                    [list v_frmbuf_wr              field_id]
			set ss_connect($ss_topology,source2target,v_frmbuf_rd,field_id)                [list v_hdmi_tx_ss             fid]
		} else {
                        if {$var_fid_enable == 1} {
                            set ss_connect($ss_topology,source2target,v_hdmi_rx_ss,fid)                [list v_tpg_ss_0               fid_in]
                        } else {
        ...
        if {[string equal -nocase $var_PROCESSOR_TYPE ARM64] != 1} {
            set ss_connect($ss_topology,source2target,vcc_const,dout)       [list bdry_out  TX_CLKSEL_OUT \
                                                                                 bdry_out  TX_EN_OUT \
                                                                                 $hdmiphy vid_phy_tx_axi4s_aresetn \
                                                                                 $hdmiphy vid_phy_rx_axi4s_aresetn]
        } else {
            set ss_connect($ss_topology,source2target,vcc_const,dout)       [list bdry_out  TX_EN_OUT \
                                                                                 $hdmiphy vid_phy_tx_axi4s_aresetn \
                                                                                 $hdmiphy vid_phy_rx_axi4s_aresetn]
        }
        ...
        if {[string equal -nocase $var_PROCESSOR_TYPE MicroBlaze] == 1} {
            ...
            if {$var_exdes_topology == 3} {
                set ss_connect($ss_topology,sourceIsInterfacePin)    [list sys_diff_clock M_AXIS DRU_CLK_IN DDC_OUT IIC M_AXI m_axi_mm_video m_axis_video m_axis_keys \
                                                                       axis_audio_out LINK_DATA_OUT vid_phy_rx_axis_ch vid_phy_status_sb_tx vid_phy_status_sb_rx \
                                                                       AUDIO_OUT VIDEO_OUT]
            } else {
            ...
            if {[regexp xilinx.com:vck190 $var_board_name match] == 1 || [regexp xilinx.com:vmk180 $var_board_name match] == 1} {
                set ss_connect($ss_topology,sourceIsInterfacePin)    [list sys_diff_clock M_AXIS DRU_CLK_IN DDC_OUT IIC M_AXI m_axi_mm_video m_axis_video m_axis_keys \
                                                                       axis_audio_out LINK_DATA_OUT vid_phy_rx_axis_ch vid_phy_status_sb_tx vid_phy_status_sb_rx \
                                                                       AUDIO_OUT VIDEO_OUT phy_data TX_REFCLK_IN HDMI_RX_CLK_IN CH_DDR]
            }
        }
    ...
    proc config_obj_property {tobe_config_obj tobe_config_obj_path} {
	    ...
	    variable var_exdes_topology
        variable config_v_frmbuf_wr
        variable config_v_frmbuf_rd		
        ...
        switch -regexp -matchvar varlist -- $tobe_config_obj {
                ...
                v_frmbuf_wr         {
                                      set config_v_frmbuf_wr(CONFIG.AXIMM_ADDR_WIDTH)  64
                                      set config_v_frmbuf_wr(CONFIG.HAS_BGR8)          1
                                      set config_v_frmbuf_wr(CONFIG.HAS_BGRX8)         1
                                      set config_v_frmbuf_wr(CONFIG.HAS_RGBX8)         1
                                      set config_v_frmbuf_wr(CONFIG.HAS_UYVY8)         1
                                      set config_v_frmbuf_wr(CONFIG.HAS_Y8)            1
                                      set config_v_frmbuf_wr(CONFIG.HAS_YUV8)          1
                                      set config_v_frmbuf_wr(CONFIG.HAS_YUVX8)         1
                                      set config_v_frmbuf_wr(CONFIG.HAS_YUYV8)         1
                                      set config_v_frmbuf_wr(CONFIG.HAS_Y_UV8)         1
                                      set config_v_frmbuf_wr(CONFIG.HAS_Y_UV8_420)     1
                                      set config_v_frmbuf_wr(CONFIG.MAX_DATA_WIDTH)    $var_c_max_bits_per_component
                                      set config_v_frmbuf_wr(CONFIG.SAMPLES_PER_CLOCK) $var_c_input_pixels_per_clock
                                      set config_v_frmbuf_wr(CONFIG.HAS_INTERLACED)    1
                                      set config_v_frmbuf_wr(CONFIG.MAX_NR_PLANES)     2
									  
                                      if {$var_c_max_bits_per_component == 10} {                          
                                        set config_v_frmbuf_wr(CONFIG.HAS_RGBX10)        1
                                        set config_v_frmbuf_wr(CONFIG.HAS_Y10)           1
					set config_v_frmbuf_wr(CONFIG.HAS_YUVX10)        1
					set config_v_frmbuf_wr(CONFIG.HAS_Y_UV10)        1
					set config_v_frmbuf_wr(CONFIG.HAS_Y_UV10_420)    1
                                      }
                                      ::ss_common::go_config $tobe_config_obj_path config_v_frmbuf_wr
                                    }
									
                v_frmbuf_rd         {
                                      set config_v_frmbuf_rd(CONFIG.AXIMM_ADDR_WIDTH)  64
                                      set config_v_frmbuf_rd(CONFIG.HAS_BGR8)          1
                                      set config_v_frmbuf_rd(CONFIG.HAS_BGRX8)         1
                                      set config_v_frmbuf_rd(CONFIG.HAS_RGBX8)         1
                                      set config_v_frmbuf_rd(CONFIG.HAS_UYVY8)         1
                                      set config_v_frmbuf_rd(CONFIG.HAS_Y8)            1
                                      set config_v_frmbuf_rd(CONFIG.HAS_YUV8)          1
                                      set config_v_frmbuf_rd(CONFIG.HAS_YUVX8)         1
                                      set config_v_frmbuf_rd(CONFIG.HAS_YUYV8)         1
                                      set config_v_frmbuf_rd(CONFIG.HAS_Y_UV8)         1
                                      set config_v_frmbuf_rd(CONFIG.HAS_Y_UV8_420)     1
                                      set config_v_frmbuf_rd(CONFIG.MAX_DATA_WIDTH)    $var_c_max_bits_per_component
                                      set config_v_frmbuf_rd(CONFIG.SAMPLES_PER_CLOCK) $var_c_input_pixels_per_clock
                                      set config_v_frmbuf_rd(CONFIG.HAS_INTERLACED)    1
                                      set config_v_frmbuf_rd(CONFIG.MAX_NR_PLANES)     2
									  
                                      if {$var_c_max_bits_per_component == 10} {                          
					set config_v_frmbuf_rd(CONFIG.HAS_RGBX10)        1
					set config_v_frmbuf_rd(CONFIG.HAS_Y10)           1
					set config_v_frmbuf_rd(CONFIG.HAS_YUVX10)        1
					set config_v_frmbuf_rd(CONFIG.HAS_Y_UV10)        1
					set config_v_frmbuf_rd(CONFIG.HAS_Y_UV10_420)    1
                                      }
									  
                                      ::ss_common::go_config $tobe_config_obj_path config_v_frmbuf_rd
                                    }									
                gt_refclk_buf       {
