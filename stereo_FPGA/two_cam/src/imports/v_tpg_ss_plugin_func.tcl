...
# BELOW ARE ALL SD SPECIFIC FUNCTIONS
    ...
    proc get_bdry_pinlist {var_connect var_topology {upvar_num 1}} {
        ...
        # ss_connect(<topology>,bdry,pinlist) [<all boundary pins for one topology>]        if {$var_fid_enable == 1} {                set sys_bdry_pinlist [list ap_clk m_axi_aresetn S_AXI_TPG S_AXI_GPIO m_axis_video fid_out]                if {$var_exdes_topology == 0 || $var_exdes_topology == 4} {                    lappend sys_bdry_pinlist s_axis_video                    lappend sys_bdry_pinlist fid_in                }        } else {                set sys_bdry_pinlist [list ap_clk m_axi_aresetn S_AXI_TPG S_AXI_GPIO m_axis_video]                if {$var_exdes_topology == 0 || $var_exdes_topology == 4} {                    lappend sys_bdry_pinlist s_axis_video                }        }
    ...
    proc get_ss_connectivity {var_connect var_topology {upvar_num 1}} {
        ...
        if {$var_exdes_topology == 0 || $var_exdes_topology == 4} {
            set ss_connect($ss_topology,source2target,bdry_in,s_axis_video)     [list v_tpg      s_axis_video]            #new feature added for 2018.3 (not backward compatible)            if {$var_fid_enable == 1} {                set ss_connect($ss_topology,source2target,bdry_in,fid_in)           [list v_tpg   fid_in]            }
        }
    ...
    proc config_obj_property {tobe_config_obj tobe_config_obj_path} {
        ...
        switch -regexp -matchvar varlist -- $tobe_config_obj {
                v_tpg               {
                                      ...
                                      if {$var_exdes_topology == 1} {
                                        set config_v_tpg(CONFIG.HAS_AXI4S_SLAVE)  0
                                      } else {
                                        set config_v_tpg(CONFIG.HAS_AXI4S_SLAVE)  1
                                      }
